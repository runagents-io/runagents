...
```json
{
  "mcpServers": {
    "runagents": {
      ...
      "env": {
        "RUNAGENTS_ENDPOINT": "https://YOUR_WORKSPACE.try.runagents.io/api/v1",
...
The MCP server exposes platform tools:
...
| `get_model_spend` | Get model spend, budgets, and warnings | Read |
| `get_agent_config` | Get agent model budgets and usage | Read |
...
| `update_agent_config` | Update agent config, including model budgets | Mutate |
